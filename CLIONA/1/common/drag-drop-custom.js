...
DHTMLgoodies_common.prototype = {
	...
	{		
	  //alert("In getTopPos inputObj.offsetTop:" + inputObj.offsetTop);
	  ...
	  //alert("In getTopPos returnValue:" + returnValue);
	  return returnValue;
	}
	// }}}
...
function DHTMLgoodies_dragDrop()
{
	...
	var okToStartDrag;				// Variable which is true or false. It would be false for 1/100 seconds after a drag has been started.
									...
									// parent element when you select dragable sub element.
...
DHTMLgoodies_dragDrop.prototype = {
	...
	__initDragDropElement : function(e)
	{
		...
		if(referenceToDragDropObject.dragDropSourcesArray[referenceToDragDropObject.numericIdToBeDragged][1]){
			referenceToDragDropObject.dragObjCloneArray[referenceToDragDropObject.numericIdToBeDragged].style.top = (dhtmlSuiteCommonObj.getTopPos(referenceToDragDropObject.dragDropSourcesArray[referenceToDragDropObject.numericIdToBeDragged][0]) - document.getElementById("content").offsetTop - document.getElementById("dropContent").offsetTop) -50 + 'px';
			referenceToDragDropObject.dragObjCloneArray[referenceToDragDropObject.numericIdToBeDragged].style.left = (dhtmlSuiteCommonObj.getLeftPos(referenceToDragDropObject.dragDropSourcesArray[referenceToDragDropObject.numericIdToBeDragged][0]) - document.getElementById("content").offsetLeft - document.getElementById("dropContent").offsetLeft) + 'px';
		}
...
function dropItems(idOfDraggedItem,targetId,x,y)
{
	...
	//Start HRB: Added for restrictig in one drop box one drag item can accomodate
	var targetObj = document.getElementById(targetId);
	if(document.getElementById("hdnQType") && document.getElementById("hdnQType").value=="1-1-dd")
	{
		var subDivs = targetObj.getElementsByTagName('DIV');
		if(subDivs.length>0 && targetId!='dropContent')return;
	}	
	//End HRB: Added for restrictig in one drop box one drag item can accomodate
	if(document.getElementById(targetId))
	{
		var obj = document.getElementById(idOfDraggedItem);
		if(obj.parentNode.id==targetId)return;		
		document.getElementById(targetId).appendChild(obj);	// Appending dragged element as child of target box
	}
	if(document.getElementById("dropContent"))
	{
		var dragabbleSubDivs = document.getElementById("dropContent").getElementsByTagName('DIV');	
		var dragDivNo = 0;
		for(dragDivNo = 0; dragDivNo < dragabbleSubDivs.length; dragDivNo++)
		{
			if(dragabbleSubDivs[dragDivNo].id.indexOf("dragDiv") != -1 && dragabbleSubDivs[dragDivNo].parentNode)
			{
				if(dragabbleSubDivs[dragDivNo].parentNode.id == "dropContent")
				{
					break;
				}
			}
		}
		if(dragDivNo == dragabbleSubDivs.length)
		{
			fnSubmitAnswer("1-1-dd");
		}
		else
