...
function iScroll (el, options) {
	...
	if(el=="adiv"){
		that.element=document.getElementById(currentDivId).childNodes["adiv"];
	}
...
iScroll.prototype = {
	...
	refresh: function () {
		...
		var parentTopPadding=that.element.parentNode.style.paddingTop?parseInt(that.element.parentNode.style.paddingTop):5;
		var parentBottomPadding=that.element.parentNode.style.paddingBottom?parseInt(that.element.parentNode.style.paddingBottom):5;
		
		that.scrollerHeight = that.element.offsetHeight+parentTopPadding+parentBottomPadding+that.options.abjustmentHeight;
		...
		that.scrollY = that.scrollerHeight >= that.scrollHeight;
	...
	setPosition: function (x, y, hideScrollBars) {
		...
		that.element.style.webkitTransform = "translateY(" +that.y + 'px)'  ;//translateOpen + that.x + 'px,' + that.y + 'px' + translateClose;
	...
	touchStart: function(e) {
		...
		//e.stopPropagation();
		try{
			if(that.element.id.toLowerCase()!="popupl1" && !isIphoneDevice){
				hideIPadPopup();
			}
		}
		catch(e){}
